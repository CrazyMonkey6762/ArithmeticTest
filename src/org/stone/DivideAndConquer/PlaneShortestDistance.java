...
public class PlaneShortestDistance {
	...
	public static double shortestDistance(int low, int height) {
		...
		/*
		 * 最短距离一定在三者中产生 1.左边的最短 2.右边的最短 3.一个在左边 一个在右边
		 */
		...
		// 对其按照y坐标排序
		Collections.sort(list, new NodeComparable8Y());
		...
		for (int i = 0; i < tempLength; i++) {
			...
			for (int j = i + 1; j < k; j++) {
				...
				if (Math.abs(list.get(j).y - list.get(i).y) >= shotestDistance) {
					break;
				}
